...
bool map_zone_mf_cache(int m, char *flag, char *params) {
	...
	} else if (!strcmpi(flag,"nobanking")) {
		if( state && map->list[m].flag.nobanking )
			;/* nothing to do */
		else {
			if( state )
				map_zone_mf_cache_add(m,"nobanking\toff");
			else if( map->list[m].flag.nobanking )
				map_zone_mf_cache_add(m,"nobanking");
		}
	}
