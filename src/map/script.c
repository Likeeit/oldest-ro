...
BUILDIN(getmapflag)
{
	...
	if(m >= 0) {
		switch(i) {
			...
			case MF_NOBANKING:         script_pushint(st,map->list[m].flag.nobanking); break;
		}
	}
...
BUILDIN(setmapflag) {
	...
	if(m >= 0) {
		switch(i) {
			...
			case MF_NOBANKING:         map->list[m].flag.nobanking = 1; break;
		}
	}
...
BUILDIN(removemapflag) {
	...
	if(m >= 0) {
		switch(i) {
			...
			case MF_NOBANKING:         map->list[m].flag.nobanking = 0; break;
		}
	}
