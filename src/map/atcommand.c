...
#define SETFLAG( cmd ) do { \
	...
	if (!message || !*message || (sscanf(message, "%99s %hd", flag_name, &flag) < 1)) {
		...
		CHECKFLAG(notomb);            CHECKFLAG(nocashshop);         CHECKFLAG(nobanking);
		clif->message(sd->fd," ");
	...
	SETFLAG(notomb);            SETFLAG(nocashshop);         SETFLAG(nobanking);
	...
	clif->message(sd->fd,"guildlock, src4instance, notomb, nocashshop, nobanking");
